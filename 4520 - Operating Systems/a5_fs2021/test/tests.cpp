...
class GradeEnvironment : public testing::Environment 
{
    public:
        virtual void SetUp() 
        {
            ...
            total = 60;	// There are really up to 63 points, so bonus points possible!
        }
...
TEST(d_tests, write_file_fill) 
{
	...
	ASSERT_EQ(fs_write(fs, fd, giant_data_chunk, BLOCK_SIZE_BYTES * 2048), BLOCK_SIZE_BYTES * 1995);
	blocks += 1995;
	//printf("*** blocks used = %lu ***\n",blocks);
	// Should be full now, so even writing 1 byte should fail
	ASSERT_EQ(fs_write(fs, fd, giant_data_chunk, 1), 0);
...
TEST(g_tests, seek) 
{
	...
	// FS_SEEK 3
	// Largest file would be: 6 blocks from direct + 2048 from indirect 
	// + 2048*2048 for double indirect = 4,196,358 blocks * 4096 bytes/block = 17,188,282,368 bytes
	// which is around 16 GB.  However, the file system itself is only 256 MB, so define that 
	// number  minus one (since the first position is position zero) as the max position.
	// In reality, we couldn't use that many blocks because we would have 63472 blocks left
	// if all of the control blocks were used on a single file (see calculation in write tests).
	// Thus, the max file size is 63472*4096 = 259,981,312.  Take this number -1 (zero-based)
	// as the max seek position possible.
	position = fs_seek(fs, fd_one, 298675309, FS_SEEK_CUR);
	ASSERT_EQ(position, 63472*BLOCK_SIZE_BYTES-1);
	score++;
...
TEST(h_tests, read) 
{
	...
	ASSERT_EQ(fs_seek(fs, fd, 0, FS_SEEK_CUR), BLOCK_SIZE_BYTES);
	score++;
